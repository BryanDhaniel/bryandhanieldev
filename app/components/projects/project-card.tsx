    ...
    >
      <Link href={`/projects/${project.slug}`} className="block h-full focus-visible:outline-offset-[-5px]">
        <div className={`relative overflow-hidden bg-ink ${featured ? "aspect-[16/10]" : "aspect-[4/3]"}`}>
          ...
          />
          <div className="absolute inset-0 z-0 bg-[linear-gradient(180deg,rgba(0,0,0,0.06)_30%,rgba(0,0,0,0.66)_100%)]" />
          {inkReveal && <InkReveal className="project-ink-reveal" maskColor={[16, 16, 16]} />}
          <span className="absolute left-4 top-4 z-10 grid h-9 w-9 place-items-center rounded-full border border-white/30 bg-ink/35 text-[10px] font-black text-white backdrop-blur-sm">
            {String(project.order).padStart(2, "0")}
          </span>
          {inkReveal && (
            <span className="project-ink-reveal-hint pointer-events-none absolute right-4 top-4 z-10 items-center rounded-full border border-white/20 bg-ink/35 px-3 py-1.5 text-[9px] font-black uppercase tracking-[0.12em] text-white/80 backdrop-blur-sm sm:right-5 sm:top-5">
              Move to reveal
            </span>
          )}
          <div className="absolute bottom-4 left-4 right-4 z-10 flex items-end justify-between gap-3 text-white sm:bottom-5 sm:left-5 sm:right-5">
            ...
            <span
              className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-paper text-ink transition-transform duration-300 group-hover:-rotate-45"
        ...
        <div className="flex min-h-48 flex-col p-5 sm:p-6">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-3 max-w-lg text-sm font-medium leading-relaxed text-ink/65">{project.description}</p>
            </div>
          </div>
          <div className="mt-auto flex items-center justify-between gap-3 border-t border-ink/10 pt-4">
            <span className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.12em] text-ink/60">
              <PiCheckCircleFill style={{ color: project.accent }} size={14} />
              {project.role}
            </span>
            <span className="shrink-0 text-[10px] font-black uppercase tracking-[0.12em] text-ink">View case study</span>
          </div>
        </div>
      </Link>
    </article>
