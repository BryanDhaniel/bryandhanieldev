...
export default async function ProjectDetailPage({ params }: ProjectPageProps) {
  ...
  return (
    <main className="min-h-screen bg-paper text-ink">
      <nav className="sticky top-0 z-30 border-b border-white/10 bg-ink/95 px-5 py-3 text-paper backdrop-blur-lg sm:px-8 lg:px-12">
        <div className="mx-auto flex max-w-[1400px] items-center justify-between gap-4">
          <Link href="/projects" className="group flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.13em] text-white/80 transition-colors hover:text-mint">
          ...
          <Link href="/#contact" className="rounded-full bg-paper px-3 py-2 text-[10px] font-black uppercase tracking-[0.11em] text-ink transition-colors hover:bg-mint sm:px-4">
            Say hello
          </Link>
        </div>
      </nav>

      <header className="grain relative overflow-hidden bg-ink px-5 pb-12 pt-14 text-paper sm:px-8 sm:pb-16 sm:pt-20 lg:px-12 lg:pb-24 lg:pt-28">
          ...
          <div className="mt-9 grid gap-10 lg:grid-cols-[1.05fr_.95fr] lg:items-end">
            ...
            <div className="flex flex-col gap-4 border-l border-white/15 pl-5 sm:flex-row sm:items-end sm:justify-between sm:gap-6 lg:flex-col lg:items-start lg:justify-end">
              ...
              <a
                ...
                className="group flex w-fit items-center gap-2 rounded-full px-4 py-2.5 text-[11px] font-black uppercase tracking-[0.12em] text-ink transition-transform hover:-translate-y-1"
      ...
      <article className="px-5 py-16 sm:px-8 sm:py-20 lg:px-12 lg:py-28">
        <div className="mx-auto grid max-w-[1180px] gap-10 lg:grid-cols-[14rem_minmax(0,1fr)] lg:gap-20">
          <aside className="lg:sticky lg:top-24 lg:h-fit">
            <p className="text-[10px] font-black uppercase tracking-[0.16em] text-ink/45">Project map</p>
            <nav className="mt-4 flex gap-2 overflow-x-auto pb-2 lg:flex-col lg:gap-1 lg:overflow-visible">
              {[
                ...
                <a key={id} href={`#${id}`} className="shrink-0 rounded-full border border-ink/10 px-3 py-2 text-[10px] font-black uppercase tracking-[0.11em] text-ink/60 transition-colors hover:border-ink hover:text-ink lg:border-0 lg:px-0 lg:py-1.5">
                  {label}
                </a>
              ))}
            </nav>
            <div className="mt-8 hidden rounded-2xl border border-ink/10 bg-stone p-4 lg:block">
              <p className="text-[10px] font-black uppercase tracking-[0.15em] text-ink/45">Snapshot</p>
              <dl className="mt-4 space-y-4 text-sm">
                <div><dt className="text-[10px] font-black uppercase tracking-[0.12em] text-ink/45">Year</dt><dd className="mt-1 font-bold">{project.year}</dd></div>
                <div><dt className="text-[10px] font-black uppercase tracking-[0.12em] text-ink/45">Format</dt><dd className="mt-1 font-bold">{project.category}</dd></div>
                <div><dt className="text-[10px] font-black uppercase tracking-[0.12em] text-ink/45">Team</dt><dd className="mt-1 font-bold leading-snug">{project.team}</dd></div>
              </dl>
            </div>
          </aside>

          <div>
            <section id="overview" className="scroll-mt-28 border-b border-ink/10 pb-12 sm:pb-16">
            ...
            <section id="intent" className="scroll-mt-28 border-b border-ink/10 py-12 sm:py-16">
              <p className="text-[10px] font-black uppercase tracking-[0.16em] text-ink/45">Why it exists</p>
              <div className="mt-5 grid gap-6 sm:grid-cols-[.28fr_.72fr]">
                ...
                <p className="max-w-2xl text-lg font-medium leading-relaxed text-ink/70 sm:text-xl">{project.purpose}</p>
              </div>
            </section>

            <section id="contribution" className="scroll-mt-28 border-b border-ink/10 py-12 sm:py-16">
              <p className="text-[10px] font-black uppercase tracking-[0.16em] text-ink/45">My contribution</p>
              <div className="mt-5 rounded-[1.5rem] bg-ink p-6 text-paper sm:p-9">
            ...
            <section id="technology" className="scroll-mt-28 py-12 sm:py-16">
              <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.16em] text-ink/45">
                <PiStackBold size={15} />
                Technology & tools
              </div>
              <div className="mt-6 flex flex-wrap gap-2.5">
                {project.stack.map((tool) => (
                  <span key={tool} className="rounded-full border border-ink/15 bg-white px-4 py-2.5 text-xs font-bold text-ink/70">{tool}</span>
                ))}
              </div>
              <a
                href={project.link}
                ...
                className="group mt-9 inline-flex items-center gap-2 text-sm font-black text-ink underline decoration-2 underline-offset-6 transition-colors hover:text-rose"
              >
      ...
      <section className="border-t border-ink/10 bg-stone px-5 py-12 sm:px-8 sm:py-16 lg:px-12">
        <div className="mx-auto grid max-w-[1400px] gap-4 md:grid-cols-2">
          <Link
            href="/projects"
            className={`group rounded-[1.5rem] border border-ink/10 bg-paper p-6 transition-transform hover:-translate-y-1 sm:p-8 ${
              next ? "" : "md:col-span-2"
            }`}
          >
            <p className="text-[10px] font-black uppercase tracking-[0.15em] text-ink/45">Project archive</p>
          ...
          {next && (
            <Link href={`/projects/${next.slug}`} className="group relative overflow-hidden rounded-[1.5rem] bg-ink p-6 text-paper transition-transform hover:-translate-y-1 sm:p-8">
